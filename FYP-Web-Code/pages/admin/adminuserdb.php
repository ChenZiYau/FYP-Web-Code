...
// Rate limit: 30 admin user operations per 15 minutes
enforce_rate_limit($pdo, 'admin_users', 30, 900);
...
// Handle edit (with CSRF and input validation)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_id'])) {
    csrf_enforce();
    $editId = validate_positive_int($_POST['edit_id']);
    $firstName = validate_string(trim($_POST['first_name'] ?? ''), 1, 50);
    $lastName = validate_string(trim($_POST['last_name'] ?? ''), 1, 50);
    $email = validate_email(trim($_POST['email'] ?? ''));
    $newPassword = trim($_POST['new_password'] ?? '');

    if ($editId === false) {
        $message = 'Invalid user ID.';
        $messageType = 'error';
    } elseif ($firstName === false || $lastName === false) {
        $message = 'First and last name are required (max 50 characters each).';
        $messageType = 'error';
    } elseif ($email === false) {
        ...
        if ($newPassword !== '') {
            $pwCheck = validate_password($newPassword);
            if ($pwCheck !== true) {
                $message = $pwCheck;
                $messageType = 'error';
            }
            ...
            <nav class="sidebar-nav">
                <div class="nav-section">
                    ...
                    <ul class="nav-menu">
                        ...
                        <li class="nav-item">
                            <a href="admin.php#website-content" class="nav-link-page">
                                <svg class="nav-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
